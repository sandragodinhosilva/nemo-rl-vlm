...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    echo "Waiting for all $EXPECTED_NODES nodes to join the Ray cluster..."
    for i in $(seq 1 180); do
        CURRENT_NODES=$(uv run python -c "import ray; ray.init(address='auto'); print(len(ray.nodes()))" 2>/dev/null || echo "0")
        echo "  Attempt $i: $CURRENT_NODES / $EXPECTED_NODES nodes connected"
