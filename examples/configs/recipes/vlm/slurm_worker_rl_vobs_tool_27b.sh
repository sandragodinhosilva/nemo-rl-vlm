#!/bin/bash
#SBATCH --job-name=rl-vobs-tool-27b
#SBATCH --nodes=1
...
# GRPO tool-loop 27B launcher — SCAFFOLD (sgsilva 2026-07-13), clone of
# slurm_worker_rl_aux_mix_4b.sh (per the handoff: never edit the 4B in place).
# ⛔ DO NOT SUBMIT until the config's two placeholders are resolved (SFT ckpt +
# vobs_tool GRPO dataset) and Sandra signs off the reward design — see
# ~/.claude/reports/post_training/2026-07-13_grpo_vobs_tool_scaffold.md §7.

set -euo pipefail
SCRIPT_PATH="/home/sgsilva/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_worker_rl_vobs_tool_27b.sh"
...
# See slurm_worker_rl_aux_mix_4b.sh for the full venv-symlink rationale (this block is a
# verbatim clone — default NEMO_RL_VENV_DIR, symlink ONLY the vLLM worker classes).
...
if [ "$NODE_RANK" -eq 0 ]; then
    CONFIG="${GRPO_CONFIG:-examples/configs/recipes/vlm/vlm_grpo_qwen35_27b_vobs_tool_thinkoff.yaml}"
    echo "=== Starting RL post-train (vobs tool-loop 27B) — config: $CONFIG ==="

    # Refuse to run on an unresolved placeholder — the scaffold's hard gate.
    if grep -q "DOES_NOT_EXIST_YET\|NOT_MATERIALIZED_YET" "$CONFIG"; then
        echo "⛔ Config still contains scaffold placeholders (SFT ckpt / vobs_tool dataset)."
        echo "   Resolve them + get Sandra's sign-off before launching (scaffold report §7)."
        exit 1
    fi

    LOG_DIR="/mnt/data/sgsilva/logs/grpo_logs/$(date +%Y%m%d)"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/rl_vobs_tool_27b_node_${NODE_RANK}_$(date +%H%M%S).log"
