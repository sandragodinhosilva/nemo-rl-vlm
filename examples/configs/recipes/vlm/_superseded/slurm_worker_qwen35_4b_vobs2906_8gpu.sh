#!/bin/bash
# SUPERSEDED 2026-07-05 — merged into ../slurm_worker_qwen35_4b_vobs2906.sh (GPU count now
# auto-derives from SLURM_GPUS_ON_NODE; override sizing via sbatch --gres/--cpus-per-task/--mem).
# Kept for provenance only — do not launch this file.
#SBATCH --job-name=sft-vlm-4b-vobs2906
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
# Full node: 8/8 GPU, so take the whole 192-CPU/2.4T node.
...
set -euo pipefail
SCRIPT_PATH="/home/sgsilva/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_worker_qwen35_4b_vobs2906_8gpu.sh"
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    # CONFIG is overridable via env so this launcher can run any 8-GPU/TP=8 variant config:
    #   CONFIG=examples/configs/sft_vlm_qwen35_4b_vobs2906_categorical_k5majority_reasoning_megatron.yaml sbatch <this>
