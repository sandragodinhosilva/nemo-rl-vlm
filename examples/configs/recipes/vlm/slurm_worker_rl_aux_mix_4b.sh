#!/bin/bash
#SBATCH --job-name=rl-mix12k-1506-4b
#SBATCH --nodes=1
...
set -euo pipefail
SCRIPT_PATH="/home/sgsilva/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_worker_rl_aux_mix_4b.sh"
...
if [ "$NODE_RANK" -eq 0 ]; then
    CONFIG="${GRPO_CONFIG:-examples/configs/recipes/vlm/vlm_grpo_qwen35_4b_mix_12k_1506.yaml}"
    echo "=== Starting RL post-train (aux mix 4B) — config: $CONFIG ==="

    LOG_DIR="/mnt/data/sgsilva/logs/grpo_logs/$(date +%Y%m%d)"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/rl_aux_mix_12k_1506_4b_node_${NODE_RANK}_$(date +%H%M%S).log"
