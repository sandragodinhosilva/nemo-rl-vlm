#!/bin/bash
#SBATCH --job-name=sft-vlm-qwen35-4b-oracle-obs-cat-reasoning
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=192
#SBATCH --exclude=worker-30,worker-31,worker-9
...
set -euo pipefail
SCRIPT_PATH="/home/sgsilva/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_4b_oracle_obs_cat_reasoning.sh"
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/training_qwen35_27b_oracle_obs_cat_reasoning_node_${NODE_RANK}_$(date +%H%M%S).log"
    ...
    # the reasoning-trace sweep can launch per-variant configs through one launcher:
    #   CONFIG=examples/configs/sft_vlm_qwen35_27b_oracle_obs_cat_reasoning_A_megatron.yaml sbatch <this>
    SFT_CONFIG="${CONFIG:-examples/configs/sft_vlm_qwen35_4b_oracle_obs_cat_reasoning_local_megatron.yaml}"
