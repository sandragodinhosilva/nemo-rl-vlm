#!/bin/bash
#SBATCH --job-name=sft-vlm-qwen35-27b-human-reps-plus-llm-fms
#SBATCH --nodes=4
...
set -euo pipefail
SCRIPT_PATH="/home/sgsilva/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_27b_human_reps_plus_llm_fms.sh"
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/training_qwen35_27b_human_reps_plus_llm_fms_node_${NODE_RANK}_$(date +%H%M%S).log"
    ...
    uv run python examples/run_vlm_sft.py \
        --config examples/configs/sft_vlm_qwen35_27b_human_reps_plus_llm_fms_megatron.yaml \
        2>&1 | tee -a "$LOG_FILE"
