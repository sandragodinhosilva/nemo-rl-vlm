#!/bin/bash
#SBATCH --job-name=sft-vlm-qwen35-27b-obs-1805-binary-aux12k-union-small25
#SBATCH --nodes=2
...
set -euo pipefail
SCRIPT_PATH="/home/sgsilva/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_27b_oracle_obs_merged_1805_binary_aux12k_union_small25.sh"
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/training_qwen35_27b_oracle_obs_merged_1805_binary_aux12k_union_small25_node_${NODE_RANK}_$(date +%H%M%S).log"
    ...
    /home/sgsilva/nemo-rl-vlm/.venv/bin/python examples/run_vlm_sft.py \
        --config examples/configs/sft_vlm_qwen35_27b_oracle_obs_merged_1805_binary_aux12k_union_small25_megatron.yaml \
        2>&1 | tee -a "$LOG_FILE"
