#!/bin/bash
#SBATCH --job-name=sft-vlm-qwen35-27b-thrive
#SBATCH --nodes=4
...
set -euo pipefail
SCRIPT_PATH="/home/sgsilva/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_27b_thrive.sh"
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    uv run python examples/run_vlm_sft.py \
        --config examples/configs/sft_vlm_qwen35_27b_mix_image10k_text5k_video10k_0804_megatron.yaml\
        2>&1 | tee -a "$LOG_FILE"
